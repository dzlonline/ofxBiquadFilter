...
void ofApp::updateMultibandNoiseDemo(){
	...
	for (int j=NOISE_DATA_STRIP_LENGTH-1; j>0; j--){
        summedFilterData[j] = summedFilterData[j-1];
    }
    summedFilterData[0] = filter.update(summedNoiseData[0]);
...
void ofApp::renderMultibandNoiseDemo(){
    ...
    render1DNoiseStrip(originX, stackBottom+225, 300,100, 0, (float *)summedFilterData);
